(() => {
      ...
      /* @__PURE__ */ React.createElement("div", { className: "tasks-scroll", style: {
        ...
        return /* @__PURE__ */ React.createElement(
          ...
          (() => {
            ...
            )), isDone ? /* @__PURE__ */ React.createElement(Icon, { name: "check", size: 15, style: { color: "var(--accent)", position: "relative" } }) : prog > 0 ? /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10.5, fontWeight: 400, color: "var(--text-muted)", position: "relative", letterSpacing: "-0.7px" } }, prog, "%") : /* @__PURE__ */ React.createElement(Icon, { name: "x", size: 11, style: { color: "rgba(255,255,255,0.22)", position: "relative" } }));
          })(),
          /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 0 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, letterSpacing: "-0.5px", color: isDone ? "var(--text-subtle)" : "var(--text)" } }, t.title), /* @__PURE__ */ React.createElement("div", { style: {
            fontSize: 11,
            color: isOverdue ? "var(--red)" : "var(--text-subtle)",
            marginTop: 2,
            letterSpacing: "-0.2px",
            whiteSpace: "nowrap",
            overflow: "hidden",
  ...
  var DriveCard = () => {
    ...
    const save = () => {
      D.setDriveConfig({ url, secret });
      toast(url.trim() ? "Conexi\xF3n con Drive guardada" : "Conexi\xF3n con Drive borrada", "success");
    };
    const missing = (D.CLIENTS || []).filter((c) => !c.driveUrl);
    const backfill = () => {
      if (!D.driveConfigured) {
        toast("Conecta Drive primero", "error");
        return;
      }
      if (!missing.length) {
        toast("Todos los clientes ya tienen carpeta", "success");
        return;
      }
  ...
  var IncomePage = () => {
    ...
    const recurringMo = activeRecs.reduce((a, r) => a + _recMoCobro(r), 0);
    ...
    const trend = (() => {
      ...
      return Array.from({ length: range }, (_, k) => {
        const d = new Date(now.getFullYear(), now.getMonth() - (range - 1 - k), 1);
        const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
        const puntual = allIncomes.filter((i) => (i.date || "").startsWith(key)).reduce((a, i) => a + _cobro(i), 0);
        const rec = activeRecs.filter((r) => recStartKey(r) <= key).reduce((a, r) => a + _recMoCobro(r), 0);
        return { key, label: MES_ES[d.getMonth()], full: `${MES_ES[d.getMonth()]} ${d.getFullYear()}`, puntual, rec, total: puntual + rec };
      });
    })();
    const _prevMo = trend[trend.length - 2], _curMo = trend[trend.length - 1];
    ...
    activeRecs.forEach((r) => {
      ...
      byClient[k] = (byClient[k] || 0) + _recMoCobro(r);
    });
